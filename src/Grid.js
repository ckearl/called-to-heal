...
const Grid = ({ user, setUser }) => {
  ...
  const labelDays = Object.keys(user.grid.days).slice(-7);
  // map through the days and return the value of each value of anxious, sad, angry, and tired
  const anxious = labelDays.map((day) => user.grid.days[day].anxious);
  const sad = labelDays.map((day) => user.grid.days[day].sad);
  const angry = labelDays.map((day) => user.grid.days[day].angry);
  const tired = labelDays.map((day) => user.grid.days[day].tired);
  ...
  return (
    <div>
      <div className="position-relative d-flex">
        ...
        <div className="position-absolute bottom-0 w-100  d-flex" style={{ height: "3rem" }}>
          {labelDays.map((day) => {
            return (
              <div
                key={day}
